...
async function scrapeFBListings() {
  ...
  while (scrollAttempts < 40) {
                ...
                const actionMatches = (text.match(/Mark as sold|Boost listing|Share|Mark out of stock|Mark as available|Edit/g) || []).length;
                ...
                if (count <= previousCount) {
                                stableRounds++;
                                if (stableRounds >= 4) { console.log('List stable after ' + scrollAttempts + ' scrolls'); break; }
                } else { stableRounds = 0; }
                previousCount = count;
                scrollAttempts++;
  }
...
function parseListings(text) {
            ...
            while (i < lines.length) {
                          if (lines[i].match(/^\$[\d,]+(\.\d{2})?$/) && i > 0) {
                                          ...
                                          const price = lines[i].replace(/\.\d{2}$/, '');
                                          let status = 'unknown';
                                          let listedDate = '';
                                          for (let j = i + 1; j < Math.min(i + 10, lines.length); j++) {
                                                            ...
                                                            if (['Mark out of stock','Mark as sold','Mark as available','Boost listing','Mark as pending','Edit'].includes(line)) break;
                                          }
